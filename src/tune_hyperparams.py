...
def run_pipeline_with_hyperparams(text, lang, use_finetuned, alpha, theta):
...
def grid_search(lang='vi', sample_count=20, dataset_name=None):
    ...
    thetas = [0.80, 0.85, 0.88, 0.90, 0.95]
    ...
    for alpha in alphas:
        for theta in thetas:
            print(f"Đang chạy cấu hình: Alpha = {alpha:.2f}, Theta = {theta:.2f}")
            rouge1_scores = []
            for sample in samples:
                article = sample['article']
                reference = sample['highlights']
                summary = run_pipeline_with_hyperparams(article, lang, True, alpha, theta)
                if summary:
                    score = scorer.score(reference, summary)
                    rouge1_scores.append(score['rouge1'].fmeasure)
            
            mean_rouge1 = np.mean(rouge1_scores) if rouge1_scores else 0.0
            results[(alpha, theta)] = mean_rouge1
    ...
    for a in alphas:
        row = [f"{a:<15.2f}"]
        for t in thetas:
            score = results[(a, t)] * 100
            row.append(f"{score:>4.2f}")
        print(" | ".join(row))

    best_params = max(results, key=results.get)
    print("="*50)
    print(f"CẤU HÌNH TỐI ƯU NHẤT: Alpha = {best_params[0]:.2f}, Theta = {best_params[1]:.2f}")
